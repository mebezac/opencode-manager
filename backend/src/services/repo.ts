...
import { createGitEnv, createNoPromptGitEnv, createGitHubGitEnv, isGitHubHttpsUrl, findGitHubCredential } from '../utils/git-auth'
...
async function executeGitWithFallback(
  ...
  const { cwd, env = createNoPromptGitEnv(), silent, database } = options
  ...
  try {
    ...
    try {
      if (isGitHubHttpsUrl(url) && database) {
        logger.warn(`Detected GitHub URL, trying stored GitHub token`)
        const settingsService = new SettingsService(database)
        const settings = settingsService.getSettings('default')
        const gitCredentials = settings.preferences.gitCredentials || []
        const githubCred = findGitHubCredential(gitCredentials)
        
        if (githubCred) {
          const ghEnv = createGitHubGitEnv(githubCred.token)
  ...
  try {
    ...
    if (shouldUseWorktree) {
      ...
      const worktreePath = path.resolve(getReposPath(), worktreeDirName)
       ...
       await executeGitWithFallback(['git', '-C', baseRepoPath, 'fetch', '--all'], { cwd: getReposPath(), env, database })
      ...
      try {
        await executeGitWithFallback(['git', 'clone', '-b', branch, normalizedRepoUrl, worktreeDirName], { cwd: getReposPath(), env, database })
      } catch (error: any) {
        ...
        if (isValidRepo.trim() === 'valid') {
          ...
          if (branch) {
            logger.info(`Switching to branch: ${branch}`)
             await executeGitWithFallback(['git', '-C', path.resolve(getReposPath(), baseRepoDirName), 'fetch', '--all'], { cwd: getReposPath(), env, database })
      ...
      try {
        ...
        await executeGitWithFallback(cloneCmd, { cwd: getReposPath(), env, database })
      } catch (error: any) {
        if (error.message.includes('destination path') && error.message.includes('already exists')) {
          logger.error(`Clone failed: directory still exists after cleanup attempt`)
          throw new Error(`Workspace directory ${worktreeDirName} already exists. Please delete it manually or contact support.`)
        ...
        await executeGitWithFallback(['git', 'clone', normalizedRepoUrl, worktreeDirName], { cwd: getReposPath(), env, database })
          let localBranchExists = 'missing'
          try {
            await executeCommand(['git', '-C', path.resolve(getReposPath(), worktreeDirName), 'rev-parse', '--verify', `refs/heads/${branch}`])
            localBranchExists = 'exists'
...
export async function listBranches(database: Database, repo: Repo): Promise<{ local: string[], all: string[], current: string | null }> {
  try {
    ...
    if (!repo.isLocal) {
      try {
        await executeGitWithFallback(['git', '-C', repoPath, 'fetch', '--all'], { env, database })
      } catch (error) {
